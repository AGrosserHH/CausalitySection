...
- `WORKSPACE_RETENTION_HOURS` (optional, default: `24`) - workspace access lifetime
- `WORKSPACE_MAX_UPLOAD_BYTES` (optional, default: `10485760`) - per-upload size cap
- `WORKSPACE_MAX_GRAPHS` (optional, default: `10`) - graphs per workspace
- `WORKSPACE_MAX_ACTIVE` (optional, default: `500`) - cap on unexpired sessions server-wide
...
that session. See [Private prototype workspaces](#private-prototype-workspaces).
...
- **Uploaded and generated files are no longer served from `/media/`.** That route now returns
  ...
  `/api/workspace/graphs/<id>/image/`. Do not configure a reverse proxy or CDN to serve `MEDIA_ROOT`.
...
| Method | URL | Description |
|--------|-----|-------------|
| GET/PATCH | `/api/workspace/session/` | Session info; shorten remaining retention |
| DELETE | `/api/workspace/session/data/` | Delete this session's graphs, uploads, cleaned copies, images, runs |
| GET | `/api/workspace/samples/` | List allowlisted guided samples |
| POST | `/api/workspace/samples/<id>/load/` | Load a sample into a new owned graph with a preset hypothesis DAG |
| GET | `/api/workspace/graphs/<id>/runs/` | Recorded analysis-stage snapshots for a graph |
| GET | `/api/workspace/graphs/<id>/image/` | Private graph image delivery |
| GET | `/api/workspace/runs/<run_id>/bundle/` | Export a run bundle; `?file_format=json` or `zip` (default) |
...
`503 session_limit` once `WORKSPACE_MAX_ACTIVE` unexpired sessions exist. Expiry and the
