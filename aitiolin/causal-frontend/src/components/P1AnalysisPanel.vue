<template>
  <details class="p1-panel">
    <summary>Reviewed comparison — P1 analysis</summary>
    ...
    <article v-if="result" aria-live="polite">
      <h3>Recorded P1 result: {{ result.specification.name }}</h3>
      ...
      <p>Use <strong>Reproducibility exports</strong> above to export this recorded P1 stage. Legacy diagnostic results are not reused for this comparison.</p>
    </article>
    <details v-if="graphId"><summary>Named runs, comparison and restore</summary>
      ...
      <label>Restore a P1 JSON/ZIP bundle using this graph's raw data <input type="file" accept=".json,.zip" :disabled="busy" @change="selectBundle" /></label>
      <p>Load the matching raw CSV first. Preview validates the graph and cleaning replay, then confirmation creates a new graph. Existing analyses are never overwritten. P0-only bundles are not imported.</p>
...
import { availableMethods, numberText, requestSpec, histogramRows } from '../p1/contracts.mjs'
...
const busy=computed(()=>working.value||p0State.pending>0)
...
function fail(e){error.value=e?.response?.data?.error||e.message||'P1 request failed.'}
function invalidate(){reviewed.value=false;stale.value=true;importPreview.value=null;importConfirmed.value=false}
watch(form,invalidate,{deep:true});watch(()=>props.graphRevision,invalidate);watch(()=>props.dataRevision,invalidate);watch(()=>p0State.seed,invalidate)
...
async function refreshSchema(){
 ...
 try{
  ...
  schema.value=(await client.get('/api/p1/schema/',{params:{graph_id:props.graphId,treatment:treatmentName.value}})).data
...
async function run(){
 const specification=requestSpec(form,{treatment:treatmentName.value,outcome:outcomeName.value,seed:p0State.seed,reviewed:reviewed.value,independent:independent.value}),state=schema.value?.state_id,gid=props.graphId
 if(!state)return
 working.value=true;error.value=''
 try{
  if(props.prepareGraph&&!(await props.prepareGraph(false)))throw new Error('Graph save failed.')
  const revision=props.graphRevision
  result.value=(await client.post('/api/p1/estimate/',{graph_id:gid,state_id:state,specification})).data
...
async function loadSynthetic(id){working.value=true;error.value='';try{emit('sample-loaded',(await client.post(`/api/p1/examples/${id}/load/`,{})).data);message.value='Synthetic data loaded. Review current data and graph before estimating.'}catch(e){fail(e)}finally{working.value=false}}
async function refreshHistory(){try{runs.value=(await client.get('/api/p1/history/',{params:{graph_id:props.graphId}})).data.runs}catch(e){fail(e)}}
async function compareRuns(){working.value=true;error.value='';try{comparison.value=(await client.post('/api/p1/compare/',{run_ids:compareIds.value})).data}catch(e){fail(e)}finally{working.value=false}}
