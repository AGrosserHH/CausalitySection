<template>
  <section class="workspace-panel" aria-label="Prototype samples, exports and privacy">
    ...
    <div class="toolbar">
      ...
      <label>Analysis seed <input v-model.number="workspaceState.seed" type="number" min="0" max="4294967295" step="1" placeholder="42 (default)" :disabled="busy" /></label>
    </div>
    ...
    <details>
      ...
      <label><input v-model="workspaceState.llmEnabled" type="checkbox" :disabled="busy || !session?.llm_available" /> Enable LLM-assisted actions, with a preview before every transmission</label>
...
import { clearSessionStorage } from "../workspace/storage.js"
import { prototypeNotice } from "../workspace/contracts.mjs"
import PrivacyReview from "./PrivacyReview.vue"
...
const busy = computed(() => workspaceState.pending > 0)
...
async function refreshSession() { session.value = (await client.get("/api/workspace/session/")).data }
onMounted(async () => {
  try { await refreshSession(); samples.value = (await client.get("/api/workspace/samples/")).data.samples }
  catch (err) { failure(err) }
})
async function loadSample() {
  error.value = ""
  try {
    const response = await client.post(`/api/workspace/samples/${sampleId.value}/load/`, {})
...
async function refreshRuns() {
  ...
  try {
    runs.value = (await client.get(`/api/workspace/graphs/${props.graphId}/runs/`)).data.runs
    runId.value = runs.value[0]?.id || ""
  } catch (err) { failure(err) }
}
watch(() => props.graphId, () => { runs.value = []; runId.value = "" })
watch(() => workspaceState.runRevision, refreshRuns)
async function download(format) {
  try {
    const response = await client.get(`/api/workspace/runs/${runId.value}/bundle/?file_format=${format}`, { responseType: "blob" })
...
async function saveRetention() {
  try { session.value = (await client.patch("/api/workspace/session/", { retention_hours: retention.value })).data }
  catch (err) { failure(err) }
}
async function deleteData() {
  ...
  try {
    await client.delete("/api/workspace/session/data/")
    clearSessionStorage(); window.location.reload()
  } catch (err) { failure(err) }
}
</script>
<style scoped>
.workspace-panel{border:1px solid var(--color-border,#ccd3dd);border-radius:8px;background:var(--color-background,#fff);padding:14px;margin:12px 0}
