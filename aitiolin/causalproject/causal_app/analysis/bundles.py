...
def validate_document(doc):
    if not isinstance(doc,dict) or doc.get('schema_version')!='aitiolin.run.v1' or not isinstance(doc.get('p1'),dict) or doc['p1'].get('schema_version')!='aitiolin.p1.v1':
        raise AnalysisError('Only run bundles that contain a reviewed comparison can be restored; workspace-only records lack an explicit comparison schema.','unsupported_bundle')
