"""Real Django integration tests for reviewed comparisons (no mocked treatment-effect calculator).
Requires the complete installed upstream repository, its dependencies and the workspace migration.
"""
...
class AnalysisApiTests(TestCase):
    def setUp(self):
        self.media=tempfile.mkdtemp(prefix='aitiolin-analysis-')
        ...
        r=self.client.post('/api/analysis/examples/randomized/load/',{},format='json');self.assertEqual(r.status_code,200,r.content);self.loaded=r.json();self.graph_id=self.loaded['graph_id']
    ...
    def run_comparison(self,spec=None,graph_id=None):
        gid=graph_id or self.graph_id;state=self.client.get('/api/analysis/schema/',{'graph_id':gid,'treatment':'Campaign'})
        self.assertEqual(state.status_code,200,state.content)
        return self.client.post('/api/analysis/estimate/',{'graph_id':gid,'state_id':state.json()['state_id'],'specification':spec or self.configuration()},format='json')
    def test_real_logistic_estimate_and_export(self):
        ...
        b=self.client.get(f"/api/workspace/runs/{r.json()['run_id']}/bundle/?file_format=json")
    ...
    def test_other_session_cannot_estimate(self):
        ...
        self.assertEqual(other.get('/api/analysis/schema/',{'graph_id':self.graph_id}).status_code,404)
    ...
    def test_stale_state_rejected(self):
        r=self.client.post('/api/analysis/estimate/',{'graph_id':self.graph_id,'state_id':'stale','specification':self.configuration()},format='json')
        self.assertEqual(r.status_code,409);self.assertEqual(r.json()['code'],'stale_state')
    def test_restore_into_new_graph_without_estimation(self):
        ...
        bundle=self.client.get(f"/api/workspace/runs/{r.json()['run_id']}/bundle/?file_format=zip").content
        def file():
            f=io.BytesIO(bundle);f.name='analysis.zip';return f
        n=CausalGraph.objects.count();preview=self.client.post('/api/analysis/bundles/preview/',{'graph_id':self.graph_id,'bundle':file()},format='multipart')
        self.assertEqual(preview.status_code,200,preview.content);self.assertEqual(CausalGraph.objects.count(),n)
        total=RunRecord.objects.count();restored=self.client.post('/api/analysis/bundles/restore/',{'graph_id':self.graph_id,'bundle':file(),'confirmed':'true','approval_digest':preview.json()['approval_digest']},format='multipart')
    ...
    def test_named_history_and_comparison(self):
        ...
        result=self.client.post('/api/analysis/compare/',{'run_ids':[a.json()['run_id'],b.json()['run_id']]},format='json')
        self.assertEqual(result.status_code,200,result.content);self.assertTrue(result.json()['comparable_scale_and_sample'])
        self.assertEqual(len(self.client.get('/api/analysis/history/',{'graph_id':self.graph_id}).json()['runs']),2)
    def test_deletion_removes_analysis_records(self):
        self.assertEqual(self.run_comparison().status_code,200)
        r=self.client.delete('/api/workspace/session/data/');self.assertEqual(r.status_code,200,r.content)
    ...
    def test_churn_real_estimator(self):
        r=self.client.post('/api/workspace/samples/churn/load/',{},format='json');self.assertEqual(r.status_code,200,r.content);gid=r.json()['graph_id']
    ...
    def test_worldbank_real_estimator(self):
        import pandas as pd
        r=self.client.post('/api/workspace/samples/worldbank/load/',{},format='json');self.assertEqual(r.status_code,200,r.content);gid=r.json()['graph_id']
