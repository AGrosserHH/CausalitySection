"""API regression tests. Run with: python manage.py test causal_app.workspace --noinput.
...
class WorkspaceApiTests(TestCase):
    def setUp(self):
        self.media = tempfile.mkdtemp(prefix="aitiolin-workspace-test-")
        self.override = override_settings(MEDIA_ROOT=self.media, OPENAI_API_KEY="",
            STORAGES={"default": {"BACKEND": "causal_app.workspace.storage.PrivateStorage"},
    ...
    def test_requires_strong_session_header(self):
        self.assertEqual(APIClient().get("/api/workspace/session/").status_code, 401)
        weak = self.new_client("guessable")
        self.assertEqual(weak.get("/api/workspace/session/").status_code, 401)
    ...
    def test_samples_are_allowlisted_and_have_no_network_dependency(self):
        names = {item["id"] for item in self.client.get("/api/workspace/samples/").json()["samples"]}
        self.assertEqual(names, {"churn", "worldbank"})
        self.assertEqual(self.client.post("/api/workspace/samples/not-a-sample/load/", {}, format="json").status_code, 404)
        with patch("openai.OpenAI") as provider:
            for sample in sorted(names):
                result = self.client.post(f"/api/workspace/samples/{sample}/load/", {}, format="json")
    ...
    def test_sample_modified_file_is_rejected(self):
        with patch("causal_app.workspace.views.manifest", return_value=[{"id": "churn", "file": "Churn.csv", "git_blob_sha1": "0" * 40}]):
            response = self.client.post("/api/workspace/samples/churn/load/", {}, format="json")
    ...
    def test_bundle_is_recorded_not_recomputed_and_excludes_data_files(self):
        ...
        run = result["run_id"]
        original = self.client.get(f"/api/workspace/runs/{run}/bundle/?file_format=zip")
        ...
        with patch("causal_app.views.estimate_effect", side_effect=AssertionError("Export must not estimate")):
            again = self.client.get(f"/api/workspace/runs/{run}/bundle/?file_format=zip")
        ...
        self.assertEqual(self.other.get(f"/api/workspace/runs/{run}/bundle/").status_code, 404)
        self.assertEqual(self.client.get(f"/api/workspace/runs/{run}/bundle/?file_format=json").status_code, 200)
        self.assertEqual(self.client.get(f"/api/workspace/runs/{run}/bundle/?file_format=bad").status_code, 400)

    def test_private_graph_image_requires_owner(self):
        graph, data = self.graph()
        result = self.estimate(graph, data)
        self.assertTrue(result["graph_image"].startswith("/api/workspace/graphs/"))
    ...
    def test_delete_removes_only_owned_data(self):
        ...
        response = self.client.delete("/api/workspace/session/data/")
    ...
    def test_expiry_blocks_access_but_allows_deletion(self):
        ...
        self.assertEqual(self.client.delete("/api/workspace/session/data/").status_code, 200)
    ...
    def test_retention_does_not_silently_extend_beyond_maximum(self):
        self.client.get("/api/workspace/session/")
        response = self.client.patch("/api/workspace/session/", {"retention_hours": 1}, format="json")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(self.client.patch("/api/workspace/session/", {"retention_hours": 8760}, format="json").status_code, 400)

    def test_purge_only_expired_sessions(self):
        self.upload(); self.upload(self.other)
        Workspace.objects.filter(pk=token_digest("ab" * 32)).update(expires_at=timezone.now() - timedelta(seconds=1))
        call_command("purge_sessions", stdout=io.StringIO())
    ...
    def test_invalid_seed_is_named_in_the_error(self):
        ...
        def with_seed(value):
            ...
            return client.get("/api/workspace/session/")
        ...
        self.assertEqual(no_header.get("/api/workspace/session/").status_code, 200)
    ...
    @override_settings(WORKSPACE_MAX_ACTIVE=2)
    def test_active_session_cap_refuses_only_new_sessions(self):
        self.assertEqual(self.client.get("/api/workspace/session/").status_code, 200)
        self.assertEqual(self.other.get("/api/workspace/session/").status_code, 200)
        third = self.new_client("ef" * 32)
        response = third.get("/api/workspace/session/")
        ...
        self.assertEqual(third.get("/api/workspace/session/").status_code, 200)
    ...
    def test_purge_removes_every_expired_session_in_one_pass(self):
        ...
        output = io.StringIO()
        call_command("purge_sessions", stdout=output)
    ...
    def test_record_keeping_routes_skip_the_rng_lock(self):
        graph, data = self.graph()
        ids = {v["name"]: v["id"] for v in data["variables"]}
        with patch("causal_app.workspace.guard.seeded", wraps=seeded) as lock:
            self.assertEqual(self.client.get(f"/api/graphs/{graph.pk}/").status_code, 200)
            self.assertEqual(self.client.get("/api/workspace/session/").status_code, 200)
