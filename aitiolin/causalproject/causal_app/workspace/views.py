...
def parse_csv(content):
    if len(content) > int(getattr(settings, "WORKSPACE_MAX_UPLOAD_BYTES", 10 * 1024 * 1024)):
...
def create_graph(request, content, title, sample=None):
    ...
    if workspace.graphs.count() >= int(getattr(settings, "WORKSPACE_MAX_GRAPHS", 10)):
    ...
    try:
        with transaction.atomic():
            ...
            graph.data_file.save(f"workspace/{uuid.uuid4().hex}.csv", ContentFile(content), save=True)
...
def upload(request):
    ...
    if file.size > int(getattr(settings, "WORKSPACE_MAX_UPLOAD_BYTES", 10 * 1024 * 1024)):
...
def variables(request):
    names = Variable.objects.filter(graph__ownership__workspace=request.workspace).values_list("name", flat=True).distinct()
...
def session_info(request):
    ...
    if request.method == "PATCH":
        ...
        maximum = workspace.created_at + timedelta(hours=max(1, min(72, int(getattr(settings, "WORKSPACE_RETENTION_HOURS", 24)))))
    ...
    return reply({"expires_at": workspace.expires_at.isoformat(),
        ...
        "data_flow": {
            ...
            "retention": "Access expires at the displayed time. Physical removal requires the scheduled purge_sessions command; closing the tab alone does not delete server files.",
...
def delete_session(request):
    return reply(delete_workspace(request.workspace))


@api_view(["GET"])
def runs(request, graph_id):
    graph = owned_graph(request.workspace, graph_id)
    records = RunRecord.objects.filter(workspace=request.workspace, graph=graph).order_by("-created_at")[:50]
...
def bundle(request, run_id):
    ...
    record = RunRecord.objects.filter(pk=run_id, workspace=request.workspace).first()
    if record is None:
        raise WorkspaceError("Run not found in this session.", 404)
    compatible = RunRecord.objects.filter(workspace=request.workspace, graph=record.graph,
...
def graph_image(request, graph_id):
    graph = owned_graph(request.workspace, graph_id)
    artifact = Artifact.objects.filter(workspace=request.workspace, graph=graph,
