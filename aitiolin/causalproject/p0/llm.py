...
def reviewed_completion(client, **kwargs):
    ...
    if request is None or not hasattr(request, "p0_workspace"):
    ...
    if token:
        try:
            permit = LLMPermit.objects.filter(pk=token, workspace=request.p0_workspace,
                operation=request.p0_operation, payload_hash=payload_hash, used=False,
    ...
    LLMPermit.objects.filter(workspace=request.p0_workspace, expires_at__lte=timezone.now()).delete()
    permit = LLMPermit.objects.create(workspace=request.p0_workspace, operation=request.p0_operation,
