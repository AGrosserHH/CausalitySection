"""P1 REST integration. Ownership, expiry and reservations are supplied by P0.
No new database schema; successful P1 runs use existing RunRecord exports.
"""
import uuid
...
from p0.core import digest, json_ready, redact
from p0.guard import reply
from p0.models import RunRecord
from p0.service import WorkspaceError, owned_graph, snapshot, environment, remember_file
from p0.views import create_graph, parse_csv
from . import P1_VERSION
...
def graph_for(request):
    ...
    return owned_graph(request.p0_workspace,value)
...
def estimate(request):
    ...
    if request.data.get('state_id')!=state:raise WorkspaceError('Saved data or graph changed. Refresh and review the comparison before running.',409,'p1_stale_state')
    spec=validate_spec(request.data.get('specification'))
    if spec['seed']!=request.p0_seed:raise AnalysisError('Workspace seed and comparison seed differ; review and retry.')
    started=timezone.now();result=analyze(frame,before['dag'],spec)
    if before['data']['effective']=='cleaned':result['warnings'].insert(0,'Existing cleaned data were used. Review earlier transformations; P1 cannot undo prior imputation or recoding.')
    analysis_key=digest({'p1_version':P1_VERSION,'snapshot':before,'specification':spec})
    record=RunRecord.objects.create(workspace=request.p0_workspace,graph=graph,operation='p1_estimate',analysis_key=analysis_key,
        payload=json_ready({'snapshot':before,'query':{k:spec[k] for k in ('treatment','outcome','control_value','treatment_value','estimand')},
          'configuration':spec,'p1':{'schema_version':'aitiolin.p1.v1','version':P1_VERSION,'specification':spec},
    ...
    return reply({**result,'p0_run_id':str(record.pk),'p0_analysis_key':analysis_key,'state_id':state})

@api_view(['GET'])
@errors
def history(request):
    graph=graph_for(request)
    records=RunRecord.objects.filter(workspace=request.p0_workspace,graph=graph,operation='p1_estimate').order_by('-created_at')[:50]
...
def compare(request):
    ...
    if not isinstance(ids,list) or len(ids)!=2:raise AnalysisError('Choose two recorded P1 runs.')
    ...
    rows=list(RunRecord.objects.filter(pk__in=ids,workspace=request.p0_workspace,operation='p1_estimate'))
    ...
    try:
        with transaction.atomic():
            ...
            if normalized['data']['effective']=='cleaned':
                graph.cleaned_file.save(f'p1-restored-{graph.pk}.csv',ContentFile(effective),save=False)
                written.append((graph.cleaned_file.storage,graph.cleaned_file.name));remember_file(request.p0_workspace,graph,graph.cleaned_file.name)
                graph.cleaning_plan=applied
            owner=graph.p0_owner;owner.cleaning_history=normalized['cleaning'];owner.save(update_fields=['cleaning_history'])
            ...
            payload.update(graph_name=graph.name,variables=[{'id':v.pk,'name':v.name} for v in variables.values()],
                ...
                outcome_id=variables[spec['outcome']].pk,method_name='backdoor.linear_regression',p1_specification=spec,
