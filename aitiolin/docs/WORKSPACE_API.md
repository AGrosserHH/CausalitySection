# Workspace API and recording contract
...
|---|---|---|
| GET | `/api/workspace/session/` | Session expiry, storage descriptions and provider availability. |
| PATCH | `/api/workspace/session/` | `{"retention_hours": 1}`; accepted options: 1, 6, 24, within the configured maximum session lifetime. |
| DELETE | `/api/workspace/session/data/` | Deletes owned records and tracked files. Busy sessions return 409; incomplete file deletion returns 500 and remains retryable. |
| GET | `/api/workspace/samples/` | Allowlisted sample titles, questions, roles, sources and caveats. |
| POST | `/api/workspace/samples/churn/load/` | Copies the bundled Churn CSV, creates an owned graph with a preset DAG and role IDs. No cleaning or LLM request. |
| POST | `/api/workspace/samples/worldbank/load/` | Same for the 2019 World Bank snapshot in original units. |
| GET | `/api/workspace/graphs/{graph_id}/runs/` | Most recent 50 stage records for an owned graph. |
| GET | `/api/workspace/runs/{uuid}/bundle/?file_format=zip` | Export stored records and matching earlier stages as a ZIP. |
| GET | `/api/workspace/runs/{uuid}/bundle/?file_format=json` | Equivalent structured JSON export. |
| GET | `/api/workspace/graphs/{graph_id}/image/` | Owned graph image, fetched with the session header and displayed from a blob URL. |
...
The patch omits the statistical profile from the model prompt and does not attach uploaded rows. Names, manually supplied context and graph descriptions can still be sensitive. The exact completion payload is shown, but it is not stored in the database; the permit stores a hash. The provider client uses a fixed OpenAI API base URL, a 30-second timeout, no SDK retries and `store=false`. This does not promise zero provider retention.
...
Session expiry blocks API access. Scheduled purge performs physical file/row deletion and skips busy sessions. Deletion is scoped to managed session records/files, never the bundled example originals. Graphs created before private workspaces existed remain unowned; no automatic ownership adoption or legacy cleanup is performed.
